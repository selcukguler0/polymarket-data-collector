#!/usr/bin/env python3
"""
Weather Collector — Order Books, Trades, Outcomes, Market Discovery

Collects data for all weather/climate markets on Polymarket:
  • weather_book_snapshots.csv  — order book snapshots for active weather markets
  • weather_live_trades.csv     — real-time trades via WebSocket
  • weather_outcomes.csv        — resolution data for settled markets
  • weather_markets.csv         — discovered active markets log
...
# ── CSV file paths ────────────────────────────────────────────────────────────
BOOK_CSV     = DATA_DIR / "weather_book_snapshots.csv"
TRADES_CSV   = DATA_DIR / "weather_live_trades.csv"
OUTCOMES_CSV = DATA_DIR / "weather_outcomes.csv"
MARKETS_CSV  = DATA_DIR / "weather_markets.csv"
...
DISCOVERY_INTERVAL   = 120    # seconds between market discovery
...
def init_csv_writers():
    ...
    book_csv = CSVAppender(BOOK_CSV, [
        ...
        "weather_tag", "event_title", "market_label", "market_slug",
    ])
    ...
    outcomes_csv = CSVAppender(OUTCOMES_CSV, [
        ...
        "weather_tag", "event_title", "market_label", "question",
    ])
...
def discover_markets():
    """Find active weather markets across all weather tags via Gamma Events API."""
            ...
            while True:
                ...
                for event in events:
                    ...
                    for mkt in event_markets:
                        ...
                        market_label = mkt.get("groupItemTitle", "") or outcomes[0]
                        ...
                        with active_markets_lock:
                            if condition_id not in active_markets:
                                ...
                                markets_csv.write_row([
                                    ...
                                    tag, event_title, market_label, question,
                                    token_yes, token_no,
...
def _fetch_one_book(token_id: str, outcome: str, mkt: dict, ts: int) -> list | None:
    ...
    for i in range(BOOK_LEVELS):
        if i < len(asks):
            ...
            row.extend([0, 0])
    row.extend([mkt["weather_tag"], mkt["event_title"], mkt["market_label"], mkt["slug"]])
    return row
    ...
    for mkt in candidates:
        ...
        if resolution:
            ...
            outcomes_csv.write_row([
                ...
                yes_price, no_price,
                mkt["weather_tag"], mkt["event_title"], mkt["market_label"], mkt["question"],
            ])
            outcomes_csv.flush()
            log.info(f"[outcome] {mkt['weather_tag']} {mkt['market_label']} → {resolution} ({cid[:12]}...)")
            ...
            while True:
                ...
                for msg in items:
                    ...
                    row = [
                        ...
                        bb, ba, bm,
                        mkt["weather_tag"], mkt["event_title"], mkt["market_label"], mkt["slug"],
                    ]
...
def run_weather_collector():
    log.info("=" * 60)
    log.info("  Weather Collector")
    log.info("  Books · Trades · Outcomes · Discovery")
    log.info(f"  Tags: {', '.join(WEATHER_TAGS)}")
    log.info("=" * 60)
    ...
    with active_markets_lock:
        ...
        for m in active_markets.values():
            by_tag[m["weather_tag"]] = by_tag.get(m["weather_tag"], 0) + 1
        log.info(f"[main] Found {len(active_markets)} active weather markets")
